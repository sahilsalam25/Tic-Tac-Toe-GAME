/*                  :: TIC-TAC-TOE GAME ::
 Tic Tac Toe is a classic two-player game.The playing board consists of a 3x3 grid, and two players take turns marking spaces on the grid with their respective symbols. Typically, one player uses "X," and the other uses "O."

The objective of the game is to form a horizontal, vertical, or diagonal line of three of one's own symbols. The player who achieves this first is declared the winner. If the entire board is filled without either player forming a line, the game is declared a draw or a tie.

This C program allows two players to play Tic Tac Toe. It includes functions to print the board, check for a win, and check for a tie. The game continues until a player wins or there is a tie. Players take turns entering their moves, and the program checks for a valid move before updating the board.
